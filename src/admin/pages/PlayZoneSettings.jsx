...
const PlayZoneSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      const { data } = await api.get('/playzone/about');
      if (data) {
        setFormData({
          ...
          aboutText: data.aboutText || data.text || '', // fallback to old text field
        });
  ...
  const handleSubmit = async (e) => {
    ...
    // Also send "text" for compatibility with PzAbout schema if we didn't remove it
    submitData.append('text', formData.aboutText);
    ...
    try {
      await api.put('/playzone/about', submitData, {
  ...
  return (
    <div className="admin-settings-page" style={{ padding: '2rem', backgroundColor: 'white', borderRadius: '8px' }}>
      <h2 style={{ marginBottom: '1.5rem' }}>Let's Play Zone Settings</h2>
...
export default PlayZoneSettings;
