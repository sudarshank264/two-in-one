...
const DoctorGalleryAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/gallery/${editingId}`, submitData, { headers: { 'Content-Type': 'multipart/form-data' } });
      } else {
        await api.post('/gallery', submitData, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.5rem' }}>
        {images.map(item => (
          <div key={item._id} style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
            <img src={getImageUrl(item.image)} alt={item.altText} style={{ width: '100%', height: '150px', objectFit: 'cover', display: 'block' }} />
...
export default DoctorGalleryAdmin;
